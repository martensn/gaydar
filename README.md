...
3. **Crosswalks from sex to gender identity** using Household Pulse Survey estimates of gender identity within age-sexuality bins, enabling analysis of gender non-binary and transgender populations. 
...
4. **
The intended use is *descriptive*: producing **expected** LGBTQ 

## Data Sources

### 1. Household Pulse Survey (HPS)

Repeated cross-sectional microdata, collected from Week 40 to 60 containing:
- Sexual orientation
- Gender identity
...
Detailed microdata allows the relationship between sexual orientation and gender identity to vary by place and age.
For example, younger gay and lesbian people are more likely to identify as non-binary than older ones. 
With microdata, my model incorporates cohort-level and spatial variation. 
---

### 2. Gallup LGBTQ Identification

[February 2026 report][[https://news.gallup.com/poll/702206/lgbtq-identification-holds.aspx] offers the most recent nationally-representative estimate of the American LGBTQ population.
Given how rapidly rates of self-identified LGBTQ identity changed in the last two decades, 
I wanted a recent estimate.
The Gallup news release reports:
- Overall national LGBTQ identification
- LGBT identification by generation
- LGBT identification by generation × sex (for select cohorts)

Given the uncertainty inherent in survey data, I use the estimates as **soft calibration targets**, not hard constraints.
In practice, most state estimates of LGBTQ identity fall within a percentage point or two
of the top-line Gallup national estimate of 9.1 percent. 
...
- \( Y_i \in \{0,1\} \): indicator for whether individual *i* identifies as LGBTQ  
- \( S_i \): sex assigned at birth  
- \( A_i \): ACS age bin  
- \( State_i \): state of residence  

The pipeline estimates:

1. **Overall LGBT identification rates**
   \[
   P(Y = 1 \mid S, State, A)
   \]

2. **Conditional category shares among LGBT individuals**
   \[
   P(Category \mid Y = 1, S, State, A)
   \]

These are combined to produce unconditional category-specific rates.
...
- LGBT indicator is defined as:
  - 1 if sexual orientation ≠ straight OR gender identity = trans.
- LGBT subcategories:
  - Lesbian or Gay
  - Bisexual
  - Queer
...
Rather than forcing exact agreement, this procedure nudges HPS-based probabilities toward Gallup benchmarks while preserving within-cell variation.
...
- \( \theta \) is chosen to minimize:

1. A deviation penalty (keeps estimates close to HPS baseline)
2. A margin penalty (penalizes differences from Gallup targets)

Optimization is performed using BFGS.

The tuning parameter \( \lambda \) controls calibration strength.
I set \( \lambda = 0.5\) because allows for some geographic concentration without
assuming all LGBTQ people live in a handful of destinations within a state. 
---
...
- Output is an adjusted probability \( p_i^{adj} \) for each respondent.
...
\[
\widehat{P}(Y=1 \mid S, State, A)
=
\frac{\sum_i w_i p_i^{adj}}{\sum_i w_i}
\]
...
- \( a \) denote exact age  
- \( g \in \{\text{cis}, \text{non\_binary}, \text{trans}\} \) index gender categories  
- \( n_{g,\ell,s,a} \) be the survey-weighted count  
- \( n_{\ell,s,a} = \sum_g n_{g,\ell,s,a} \)  
- \( \hat{p}_{g,\ell,s,a} = n_{g,\ell,s,a} / n_{\ell,s,a} \)
...
The age-local prior mean is the weighted average:
...
A Dirichlet prior is centered at \( \mathbf{m}^{(a)} = (m_g^{(a)}) \).

The concentration parameter is estimated by method of moments:
...
where \( v_g \) is the weighted variance of \( \hat{p}_{g,\ell,s,a'} \) across neighbor ages.
...
=
\frac{
n_{\text{eff},\ell,s,a} \hat{p}_{g,\ell,s,a}
+
\alpha_g
}{
n_{\text{eff},\ell,s,a}
...
- Shrinkage strength adapts to effective sample size  
...
For each sex \( s \), ACS age bin \( a \), and LGBT category \( \ell \), define:
...
with parameters chosen to match the weighted cross-state mean and variance.
...
=
\frac{
n_{\text{eff},j} \hat{\pi}_{\ell,s,a,j}
+
\alpha
}{
n_{\text{eff},j}
